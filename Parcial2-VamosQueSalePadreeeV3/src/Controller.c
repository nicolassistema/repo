	...
	if(list != NULL && id > 0 && (choiceList == 1 || choiceList == 2))
	{
		for (i = 0; i < ll_len(list); i++)
		{
			if(choiceList == CLIENT)
			{
				...
				pElement = (Afiche*)ll_get(list,i);
				resultAux = afiche_getId(pElement,&bufferId);
			}
			if(resultAux != -1 && bufferId == id)
			{
				result = pElement;
...
Afiche* info_qtyPostersBySale(LinkedList* listSale, int choice, int* qtyPoster, int status)
{
	void* returnAux = NULL;
	Afiche* pSale;
	int qty;
	...
	if(listSale != NULL && (choice == MAX || choice == MIN) && qtyPoster != NULL && (status == TO_CHARGE || status == CHARGED))
	{
		for(i = 0; i< ll_len(listSale);i++)
		{
			...
			if(pSale != NULL)
			{
				afiche_getCantidadAfiches(pSale,&qty);
				afiche_getEstadoNum(pSale,&stat);
...
{
	Afiche* pSale = (Afiche*)pElement;
	int* idClient = (int*)arg;
	int bufferId;
	int qty = 0;

	afiche_getIdCliente(pSale, &bufferId);
...
int sale_compareByStatus(void* this, void* arg)
{
	...
	if(this != NULL)
	{
		afiche_getEstadoNum(status,&bufferStatus);
 ...
 * 				  NULL - Error
 */
Cliente* info_qtyPostersByClient(LinkedList* listSale,LinkedList* listClient, int choice, int* qtyPoster,int status)
{
	void* returnAux = NULL;
				...
				(choice == MAX || choice == MIN) && qtyPoster != NULL && (status == TO_CHARGE || status == CHARGED))
	{
		...
		{
			if(ll_filter2(clonedListSale,sale_compareByStatus,&status) == 0)
			{
	...
	if(listSale != NULL && listClient != NULL)
	{
		do
		{
			if(utn_getNumero(&option,"\nIngrese una opcion: "
								...
								"\nError, elija una opcion valida!",1,4,5)==0)
